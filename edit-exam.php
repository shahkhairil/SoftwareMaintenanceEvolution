...
$sql="update tblresult  set ExamId=:examid where id=:iid ";
...
$ret = "SELECT tblstudents.StudentName,tblclasses.ClassName,tblclasses.Section,tblresult.ExamId,tblexam.ExamName from tblresult  join tblexam on tblresult.ExamId=tblexam.ExamID join tblstudents on tblresult.StudentId=tblresult.StudentId join tblsubjects on tblsubjects.id=tblresult.SubjectId join tblclasses on tblclasses.id=tblstudents.ClassId where tblstudents.StudentId=:stid limit 1";
...
<br>
  ...
  <option value="<?php echo htmlentities($row->ExamId); ?>"><?php echo htmlentities($row->ExamName); ?></option>
